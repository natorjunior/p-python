...
(function () {
    ...
    const FLIGHT_EVENT_CONFIG = [
        ...
        { id: "vento", col: 27, row: 4, type: "totem", label: "Torre de Vento" },
        { id: "ninho", col: 36, row: 6, type: "nest", label: "Ninho Celeste" },
        { id: "orb_b", col: 46, row: 3, type: "crystal", label: "Orbital B" },
        { id: "corrente", col: 56, row: 5, type: "totem", label: "Corrente Sul" },
        { id: "orb_c", col: 66, row: 4, type: "crystal", label: "Orbital C" },
    ];
    ...
    function buildRandomGroundEvent() {
        var crystal = Crafty.e("2D, DOM, Image")
            .attr({ x: colToX(RANDOM_EVENT_COL) + 16, y: rowToY(GROUND_ROW) - 44, w: 32, h: 44, z: 31 })
        ...
        var obstacleX = colToX(RANDOM_EVENT_COL) + (TILE - 34) / 2;
        var obstacleY = rowToY(GROUND_ROW) - 46;
        var obstacleBody = Crafty.e("2D, DOM, Color")
            .attr({ x: obstacleX, y: obstacleY, w: 34, h: 46, z: 31 })
            .color("#65466f")
            .css({ "border-radius": "8px" });
        var obstacleCap = Crafty.e("2D, DOM, Color")
            .attr({ x: obstacleX + 3, y: obstacleY + 3, w: 28, h: 8, z: 32 })
            .color("#f38ba8")
            .css({ "border-radius": "4px", opacity: "0.85" });
        var obstacleSymbol = Crafty.e("2D, DOM, Text")
            .attr({ x: obstacleX, y: obstacleY + 14, w: 34, h: 16, z: 33 })
            .text("!")
            .textColor("#f9e2af")
            .textFont({ size: "16px", weight: "700", family: "Consolas" })
            .css({ "text-align": "center" });

        var interactMarker = createInteractionMarker(RANDOM_EVENT_COL, obstacleY - 18, "COLETA?");
        var jumpMarker = createJumpMarker(RANDOM_EVENT_COL, obstacleY - 18);

        randomGroundEvent = {
            col: RANDOM_EVENT_COL,
            type: "crystal",
            resolved: false,
            crystal: crystal,
            obstacleBody: obstacleBody,
            obstacleCap: obstacleCap,
            obstacleSymbol: obstacleSymbol,
            interactMarker: interactMarker,
            jumpMarker: jumpMarker,
            setType: function (nextType) {
                this.type = nextType === "obstacle" ? "obstacle" : "crystal";

                var isCrystal = this.type === "crystal";
                setEntityVisible(this.crystal, isCrystal);
                setEntityVisible(this.obstacleBody, !isCrystal);
                setEntityVisible(this.obstacleCap, !isCrystal);
                setEntityVisible(this.obstacleSymbol, !isCrystal);
                setEntityVisible(this.interactMarker.bg, isCrystal);
                setEntityVisible(this.interactMarker.text, isCrystal);
                setEntityVisible(this.interactMarker.pulse, isCrystal);
                setEntityVisible(this.jumpMarker.bg, !isCrystal);
                setEntityVisible(this.jumpMarker.text, !isCrystal);
            },
            resolveByInteract: function () {
                this.resolved = true;
                this.crystal.css({ opacity: "0.35", filter: "saturate(0.7)" });
                setInteractionMarkerState(this.interactMarker, "done");
            },
            resolveByJump: function () {
                this.resolved = true;
                this.obstacleBody.color("#4f5d73");
                this.obstacleCap.color("#a6e3a1");
                this.obstacleSymbol.text("✓").textColor("#a6e3a1");
                this.jumpMarker.bg.color("#20394a").css({ border: "1px solid rgba(166,227,161,0.55)", opacity: "0.95" });
                this.jumpMarker.text.text("DESVIO ✓").textColor("#a6e3a1");
            },
            reset: function () {
                this.resolved = false;
                this.crystal.css({ opacity: "1", filter: "none" });
                this.obstacleBody.color("#65466f").css({ filter: "none" });
                this.obstacleCap.color("#f38ba8").css({ filter: "none" });
                this.obstacleSymbol.text("!").textColor("#f9e2af").css({ filter: "none" });
                setInteractionMarkerState(this.interactMarker, "default");
                this.jumpMarker.bg.color("#321b38").css({ border: "1px solid rgba(243,139,168,0.45)", opacity: "0.92" });
                this.jumpMarker.text.text("JUMP").textColor("#f38ba8");
                this.setType(Math.random() < 0.5 ? "crystal" : "obstacle");
            },
        };

        randomGroundEvent.reset();
    }
    ...
    function allInteractionsDone() {
        ...
        if (randomGroundEvent && !randomGroundEvent.resolved) return false;
        for (var i = 0; i < interactions.length; i++) {
    ...
    function updateInteractionGuidance() {
        ...
        var randomInRange = !!randomGroundEvent && !randomGroundEvent.resolved && Math.abs(randomGroundEvent.col - currentCol) <= 1;

        if (randomGroundEvent && !randomGroundEvent.resolved) {
            if (randomGroundEvent.type === "crystal") {
                setInteractionMarkerState(randomGroundEvent.interactMarker, randomInRange ? "focus" : "default");
            } else {
                randomGroundEvent.jumpMarker.bg.color(randomInRange ? "#3f2a45" : "#321b38")
                    .css({ border: randomInRange ? "1px solid rgba(249,226,175,0.62)" : "1px solid rgba(243,139,168,0.45)", opacity: "0.92" });
                randomGroundEvent.jumpMarker.text.textColor(randomInRange ? "#f9e2af" : "#f38ba8");
            }
        }
    ...
    function updateObjectiveHud() {
        ...
        if (randomGroundEvent && !randomGroundEvent.resolved) {
            randomStatus = "⏳ if is_crystal_ahead()";
        }
    ...
    function checkVictory() {
        ...
        showPlayerDialog("Você resolveu o checkpoint aleatório, coletou a montaria, ativou os pontos e concluiu o voo.", "success", 3400, {
            title: "VITÓRIA",
            hint: "Fase 12 finalizada com sucesso.",
        });
        termLog("✓ Vitória! Fase 12 concluída.", "term-success");
        updateErrorStatus(0);
    ...
    function resetRandomGroundEvent() {
        if (!randomGroundEvent) return;
        randomGroundEvent.reset();
    }
    ...
    async function moveSteps(direction, steps, id, moveSpeed, lineNum) {
        for (var s = 0; s < steps; s++) {
            ...
            if (!isMounted && randomGroundEvent && !randomGroundEvent.resolved && nextCol > RANDOM_EVENT_COL + 1) {
                throw new Error("Ln " + lineNum + ": resolva o checkpoint aleatório com if is_crystal_ahead() antes de avançar.");
            }

            if (!isMounted && isGroundBlockedCol(nextCol)) {
    ...
    async function runJump(id, moveSpeed, lineNum) {
        ...
        if (!isGroundBlockedCol(obstacleCol)) {
            await hopInPlace(id);
        ...
        if (isGroundBlockedCol(landingCol)) {
        ...
        if (isRandomObstacleAt(obstacleCol)) {
            randomGroundEvent.resolveByJump();
            showPlayerDialog("Obstáculo aleatório desviado com sucesso.", "success", 2200, {
                title: "CHECKPOINT RESOLVIDO",
                hint: "Agora siga para a montaria.",
            });
            termLog("✓ Checkpoint: obstáculo desviado.", "term-success");
        }
        updateInteractionGuidance();
        updateObjectiveHud();
    }
    ...
    async function runInteract(lineNum, id, moveSpeed) {
        ...
        if (randomGroundEvent && !randomGroundEvent.resolved) {
            if (randomGroundEvent.type === "crystal" && Math.abs(currentCol - randomGroundEvent.col) <= 1) {
                randomGroundEvent.resolveByInteract();
                showPlayerDialog("Cristal coletado. Checkpoint aleatório resolvido.", "success", 2300, {
                    title: "CHECKPOINT RESOLVIDO",
                    hint: "Se fosse obstáculo, o caminho correto seria jump().",
                });
                termLog("✓ Checkpoint: cristal coletado.", "term-success");
                updateInteractionGuidance();
                updateObjectiveHud();
                return;
            }

            if (randomGroundEvent.type === "obstacle" && Math.abs(currentCol - randomGroundEvent.col) <= 1) {
                showPlayerDialog("Aqui é obstáculo. Use jump() para desviar.", "error", 2300, {
                    title: "CHECKPOINT ALEATÓRIO",
                    hint: "Use if is_crystal_ahead(): ... else: jump()",
                });
                termLog("… Ln " + lineNum + ": evento aleatório é obstáculo; use jump() para desviar.", "term-output");
                return;
            }
        }
    ...
    function isRandomObstacleAt(col) {
        return !!randomGroundEvent &&
            !isMounted &&
            !randomGroundEvent.resolved &&
            randomGroundEvent.type === "obstacle" &&
            randomGroundEvent.col === col;
    }

    function isGroundBlockedCol(col) {
        return !!OBSTACLE_SET[col] || isRandomObstacleAt(col);
    }
    ...
    function isCrystalAhead() {
        return !!randomGroundEvent &&
            !isMounted &&
            !randomGroundEvent.resolved &&
            randomGroundEvent.type === "crystal" &&
            randomGroundEvent.col === currentCol + facing;
    }

    function canInteractNow() {
        if (!isMounted && randomGroundEvent && !randomGroundEvent.resolved && randomGroundEvent.type === "crystal") {
            return Math.abs(currentCol - randomGroundEvent.col) <= 1;
        }
        if (isMounted) {
            return !!getNearestFlightInteraction(1);
        }
        return !!mountPoint && !mountPoint.done && Math.abs(currentCol - MOUNT_COL) <= 1;
    }

    function parseIndentedBranchCommand(rawLine, lineNum, ifLine) {
        if (!/^\s+/.test(rawLine)) {
            throw new Error("Ln " + lineNum + ": bloco do if na linha " + ifLine + " exige indentação.");
        }
        var command = rawLine.trim();

        var rightMatch = command.match(/^move_right\s*\(\s*(\d+)\s*\)$/);
        if (rightMatch) {
            ...
            return { type: "right", steps: rightSteps, line: lineNum };
        }

        var leftMatch = command.match(/^move_left\s*\(\s*(\d+)\s*\)$/);
        if (leftMatch) {
            ...
            return { type: "left", steps: leftSteps, line: lineNum };
        }

        if (/^interact\s*\(\s*\)$/.test(command)) return { type: "interact", line: lineNum };
        if (/^jump\s*\(\s*\)$/.test(command)) return { type: "jump", line: lineNum };

        throw new Error("Ln " + lineNum + ": use move_right(n), move_left(n), interact() ou jump() dentro deste if.");
    }
    ...
    function parseCode(code) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (/^if\s+is_crystal_ahead\s*\(\s*\)\s*:\s*$/.test(line)) {
                var ifIndent = (lines[i].match(/^\s*/) || [""])[0].length;
                var j = i + 1;
                var elseLineNum = -1;
                var thenCommands = [];
                ...
                while (j < lines.length) {
                    var rawThen = lines[j];
                    var trimmedThen = rawThen.trim();
                    var indentThen = (rawThen.match(/^\s*/) || [""])[0].length;
                    ...
                    thenCommands.push(parseIndentedBranchCommand(rawThen, j + 1, lineNum));
                    j++;
                }
                ...
                while (j < lines.length) {
                    var rawElse = lines[j];
                    var trimmedElse = rawElse.trim();
                    var indentElse = (rawElse.match(/^\s*/) || [""])[0].length;
                    ...
                    elseCommands.push(parseIndentedBranchCommand(rawElse, j + 1, lineNum));
                    j++;
                }
            ...
            if (/^if\s+can_interact\s*\(\s*\)\s*:\s*$/.test(line)) {
                if (i + 1 >= lines.length) {
                    throw new Error("Ln " + lineNum + ": bloco if incompleto. Use uma linha indentada com interact().");
                }

                var bodyRaw = lines[i + 1];
                var bodyLineNum = i + 2;

                if (!/^\s+interact\s*\(\s*\)\s*$/.test(bodyRaw)) {
                    if (/^interact\s*\(\s*\)\s*$/.test(bodyRaw.trim())) {
                        throw new Error("Ln " + bodyLineNum + ": após if can_interact(): use indentação antes de interact().");
                    }
                    throw new Error("Ln " + bodyLineNum + ": este if simples aceita apenas interact() indentado.");
                }

                commands.push({ type: "if_interact", line: lineNum, bodyLine: bodyLineNum });
                i += 1;
                continue;
            }
    ...
    async function executeBranchCommand(cmd, id, moveSpeed) {
        ...
        throw new Error("Ln " + cmd.line + ": comando inválido dentro do bloco if.");
    }
    ...
    async function executeCommand(cmd, id, moveSpeed) {
        ...
        if (cmd.type === "right") {
            ...
            await runInteract(cmd.line, id, moveSpeed);
        } else if (cmd.type === "if_interact") {
            termLogHtml(
                '<span class="term-prompt">❯</span> ' +
                '<span class="term-cmd">Linha ' + cmd.line + ':</span> if can_interact():'
            );
            if (canInteractNow()) {
                highlightEditorLine(cmd.bodyLine);
                await runInteract(cmd.bodyLine, id, moveSpeed);
            } else {
                termLog("… Ln " + cmd.line + ": condição falsa, interact() não executado.", "term-output");
            }
            ...
            if (isCrystalAhead()) {
                for (var t = 0; t < cmd.thenCommands.length; t++) {
                    await executeBranchCommand(cmd.thenCommands[t], id, moveSpeed);
                ...
                for (var e = 0; e < cmd.elseCommands.length; e++) {
                    await executeBranchCommand(cmd.elseCommands[e], id, moveSpeed);
                    if (hasWon || id !== execId) return;
                }
            }
