/* ===== FASE 7 — Cenário Dinâmico + Câmera ===== */
...
(function () {
    ...
    function createInteractionMarker(col, yTop, label) {
        ...
        var pulse = Crafty.e("2D, DOM, Color")
            .attr({ x: centerX - 18, y: rowToY(GROUND_ROW) + 2, w: 36, h: 10, z: 29 })
            .color("#89dceb")
    ...
    function createInteractionNpc(config) {
        var col = config.col;
        var accentColor = config.accentColor || "#89dceb";
        var w = config.w || 44;
        var h = config.h || 54;
        var x = colToX(col) + Math.round((TILE - w) / 2) + (config.xOffset || 0);
        var y = rowToY(GROUND_ROW) - h + (config.yOffset || 0);
        var badgeW = config.badgeW || 56;
        var badgeX = colToX(col) + Math.round((TILE - badgeW) / 2);
        var z = config.z || 30;

        var npc = Crafty.e("2D, DOM, Image")
            .attr({ x: x, y: y, w: w, h: h, z: z })
            .image(config.image, "no-repeat")
            .css({
        ...
        var aura = Crafty.e("2D, DOM, Color")
            .attr({ x: x, y: rowToY(GROUND_ROW) + 2, w: w, h: 10, z: z - 1 })
            .color(accentColor)
            .css({ opacity: "0.16", "border-radius": "50%" });

        var badge = Crafty.e("2D, DOM, Color")
            .attr({ x: badgeX, y: y - 16, w: badgeW, h: 14, z: z + 1 })
            .color("#111b31")
            .css({
                "border-radius": "7px",
        ...
        var badgeText = Crafty.e("2D, DOM, Text")
            .attr({ x: badgeX, y: y - 14, w: badgeW, h: 10, z: z + 2 })
            .text(config.badgeText || "INT")
            .textColor(accentColor)
            .textFont({ size: "9px", weight: "700", family: "Consolas" })
            .css({ "text-align": "center", display: "none" });
    ...
    function createTerminalStation(col, accentColor) {
        var baseX = colToX(col) + 13;
        var baseY = rowToY(GROUND_ROW) - 45;

        var body = Crafty.e("2D, DOM, Color")
            .attr({ x: baseX, y: baseY, w: 38, h: 30, z: 30 })
            .color("#22324f")
            .css({ "border-radius": "7px", border: "1px solid rgba(201,215,255,0.22)" });

        var screen = Crafty.e("2D, DOM, Color")
            .attr({ x: baseX + 4, y: baseY + 4, w: 30, h: 16, z: 31 })
            .color("#081225")
            .css({ "border-radius": "4px", border: "1px solid rgba(110,231,255,0.35)" });

        var prompt = Crafty.e("2D, DOM, Text")
            .attr({ x: baseX + 4, y: baseY + 7, w: 30, h: 12, z: 32 })
            .text(">_")
            .textColor("#7dcfff")
            .textFont({ size: "10px", weight: "700", family: "Consolas" })
            .css({ "text-align": "center" });

        var feet = Crafty.e("2D, DOM, Color")
            .attr({ x: baseX + 12, y: baseY + 30, w: 14, h: 6, z: 30 })
            .color("#1b263e")
            .css({ "border-radius": "3px" });

        var aura = Crafty.e("2D, DOM, Color")
            .attr({ x: baseX, y: rowToY(GROUND_ROW) + 2, w: 38, h: 10, z: 29 })
            .color(accentColor)
            .css({ opacity: "0.16", "border-radius": "50%" });

        var badge = Crafty.e("2D, DOM, Color")
            .attr({ x: colToX(col) + 4, y: baseY - 16, w: 56, h: 14, z: 33 })
            .color("#111b31")
            .css({
                "border-radius": "7px",
                border: "1px solid " + accentColor,
        ...
        var badgeText = Crafty.e("2D, DOM, Text")
            .attr({ x: colToX(col) + 4, y: baseY - 14, w: 56, h: 10, z: 34 })
            .text("INT")
            .textColor(accentColor)
            .textFont({ size: "9px", weight: "700", family: "Consolas" })
            .css({ "text-align": "center", display: "none" });
        ...
        return {
            ...
            badge: badge,
            badgeText: badgeText,
            extras: [screen, prompt, feet],
        };
    }

    function setInteractionNpcGlow(npcObj, filter) {
        if (!npcObj) return;
        npcObj.npc.css({ filter: filter });
        var extras = npcObj.extras || [];
        for (var i = 0; i < extras.length; i++) {
            extras[i].css({ filter: filter });
        }
    }
    ...
    function buildInteractions() {
        ...
        // Crystal relay
        var crystalNpc = createInteractionNpc({
            col: 6,
            image: "../assets/img/cristal.png",
            accentColor: "#7dcfff",
            w: 34,
            h: 50,
            badgeW: 58,
        ...
        var crystalMarker = createInteractionMarker(6, rowToY(GROUND_ROW) - 70, "INTERACT");

        interactions.push({
            id: "crystal",
            label: "Cristal de Energia",
            col: 6,
            marker: crystalMarker,
            dialog: "Cristal sincronizado. Energia de rota estabilizada.",
            hint: "Ponto de energia ativado. Continue pela rota.",
            done: false,
            activate: function () {
                setInteractionNpcGlow(crystalNpc, "drop-shadow(0 0 12px rgba(166,227,161,0.55)) saturate(0.9)");
                crystalNpc.aura.color("#a6e3a1").css({ opacity: "0.52" });
                crystalNpc.badge.css({ border: "1px solid #a6e3a1" });
                crystalNpc.badgeText.text("OK").textColor("#a6e3a1");
                setInteractionMarkerState(crystalMarker, "done");
            },
            reset: function () {
                setInteractionNpcGlow(crystalNpc, "none");
                crystalNpc.aura.color("#7dcfff").css({ opacity: "0.16" });
                crystalNpc.badge.css({ border: "1px solid #7dcfff" });
                crystalNpc.badgeText.text("CRISTAL").textColor("#7dcfff");
                setInteractionMarkerState(crystalMarker, "default");
            },
        });

        // NPC mentor
        var mentorNpc = createInteractionNpc({
            col: 20,
            image: "../assets/img/playerMulher.png",
            accentColor: "#89dceb",
            w: 44,
            h: 54,
            badgeW: 56,
        ...
        var mentorMarker = createInteractionMarker(20, rowToY(GROUND_ROW) - 74, "INTERACT");

        interactions.push({
            id: "mentor",
            label: "Mentora da Rota",
            col: 20,
            marker: mentorMarker,
            dialog: "Mentoria concluída. Próximo ponto já está mapeado.",
            hint: "Dica recebida. Procure o console adiante.",
            done: false,
            activate: function () {
                setInteractionNpcGlow(mentorNpc, "drop-shadow(0 0 12px rgba(166,227,161,0.55)) saturate(0.9)");
                mentorNpc.aura.color("#a6e3a1").css({ opacity: "0.52" });
                mentorNpc.badge.css({ border: "1px solid #a6e3a1" });
                mentorNpc.badgeText.text("OK").textColor("#a6e3a1");
                setInteractionMarkerState(mentorMarker, "done");
            },
            reset: function () {
                setInteractionNpcGlow(mentorNpc, "none");
                mentorNpc.aura.color("#89dceb").css({ opacity: "0.16" });
                mentorNpc.badge.css({ border: "1px solid #89dceb" });
                mentorNpc.badgeText.text("MENTOR").textColor("#89dceb");
                setInteractionMarkerState(mentorMarker, "default");
            },
        });

        // Console terminal
        var terminalNpc = createTerminalStation(31, "#f9e2af");
        var terminalMarker = createInteractionMarker(31, rowToY(GROUND_ROW) - 70, "INTERACT");

        interactions.push({
            id: "terminal",
            label: "Console de Navegação",
            col: 31,
            marker: terminalMarker,
            dialog: "Console ligado. Coordenadas intermediárias registradas.",
            hint: "Rota parcial registrada. Falta sincronizar o baú.",
            done: false,
            activate: function () {
                setInteractionNpcGlow(terminalNpc, "drop-shadow(0 0 12px rgba(166,227,161,0.55)) saturate(0.9)");
                terminalNpc.aura.color("#a6e3a1").css({ opacity: "0.52" });
                terminalNpc.badge.css({ border: "1px solid #a6e3a1" });
                terminalNpc.badgeText.text("OK").textColor("#a6e3a1");
                setInteractionMarkerState(terminalMarker, "done");
            },
            reset: function () {
                setInteractionNpcGlow(terminalNpc, "none");
                terminalNpc.aura.color("#f9e2af").css({ opacity: "0.16" });
                terminalNpc.badge.css({ border: "1px solid #f9e2af" });
                terminalNpc.badgeText.text("CONSOLE").textColor("#f9e2af");
                setInteractionMarkerState(terminalMarker, "default");
            },
        });

        // Chest checkpoint
        var chestNpc = createInteractionNpc({
            col: 41,
        ...
        var chestMarker = createInteractionMarker(41, rowToY(GROUND_ROW) - 68, "INTERACT");

        interactions.push({
            id: "chest",
            label: "Baú de Sincronização",
            col: 41,
            marker: chestMarker,
            dialog: "Baú sincronizado. Rota final desbloqueada.",
            hint: "Tudo pronto. Agora a meta será liberada.",
            done: false,
            activate: function () {
                setInteractionNpcGlow(chestNpc, "drop-shadow(0 0 12px rgba(166,227,161,0.55)) saturate(0.9)");
                chestNpc.aura.color("#a6e3a1").css({ opacity: "0.52" });
                chestNpc.badge.css({ border: "1px solid #a6e3a1" });
                chestNpc.badgeText.text("OK").textColor("#a6e3a1");
                setInteractionMarkerState(chestMarker, "done");
            },
            reset: function () {
                setInteractionNpcGlow(chestNpc, "none");
                chestNpc.aura.color("#fab387").css({ opacity: "0.16" });
                chestNpc.badge.css({ border: "1px solid #fab387" });
                chestNpc.badgeText.text("BAÚ").textColor("#fab387");
                setInteractionMarkerState(chestMarker, "default");
            },
        });
    }
    ...
    function updateInteractionGuidance() {
        var focus = getClosestPendingInteraction();
        for (var i = 0; i < interactions.length; i++) {
            var item = interactions[i];
            if (item.done) {
    ...
    function unlockGoalIfReady() {
        ...
        goalLocked = false;
        updateGoalVisual();
        showPlayerDialog("Todos os NPCs ativados. Meta liberada!", "success", 3000, {
            title: "OBJETIVO",
            hint: "Siga até a bandeira para concluir a fase.",
        });
    ...
    function checkVictory() {
        ...
        showPlayerDialog("Fase 7 concluída com sucesso.", "success", 3200, {
            title: "VITÓRIA",
            hint: "Você concluiu todas as interações da Fase 7.",
        });
        termLog("✓ Vitória! Fase 7 concluída.", "term-success");
        updateErrorStatus(0);
    ...
    async function runInteract(lineNum) {
        ...
        if (!item) {
            showPlayerDialog("Aproxime-se de um NPC de interação para usar interact().", "error", 2400, {
                title: "INTERAÇÃO",
                hint: "Pare ao lado de um ponto marcado com INTERACT.",
            });
            termLog("… Ln " + lineNum + ": aproxime-se de um NPC de interação para usar interact().", "term-output");
            return;
        }
        ...
        updateInteractionGuidance();
        updateObjectiveHud();
        showPlayerDialog(item.dialog || ("Interação ativada: " + item.label + "."), "info", 2800, {
            title: item.label || "INTERAÇÃO",
            hint: item.hint || "Continue para o próximo ponto de interação.",
        });

        termLog("✓ Interação ativada: " + item.label + ".", "term-success");
        unlockGoalIfReady();
        await sleep(120);
    }
