...
(function () {
    ...
    const COLS = 14;
    const ROWS = 6;
    const VIEW_COLS = Math.max(10, COLS - 2);
    ...
    let nextPatternIndex = Math.random() < 0.5 ? 0 : 1;
    ...
    function cameraTargetX() {
        ...
        const raw = -(center - VIEW_W / 2);
        const minX = Math.min(0, VIEW_W - GAME_W);
        return snapPixel(clamp(raw, minX, 0));
    }

    function setCameraImmediate() {
        Crafty.viewport.x = snapPixel(cameraTargetX());
        Crafty.viewport.y = 0;
    }

    function updateCameraSmooth() {
        const targetX = cameraTargetX();
        Crafty.viewport.x = snapPixel(Crafty.viewport.x + (targetX - Crafty.viewport.x) * 0.18);
    ...
    /* ===== RANDOM EVENTS ===== */
    function buildRandomEvent(col) {
        const crystalX = colToX(col) + (TILE - 30) / 2;
        const crystalY = rowToY(GROUND_ROW) - 38;
        const crystal = Crafty.e("2D, DOM, Image")
        ...
        const baseX = colToX(col) + (TILE - 34) / 2;
        const baseY = rowToY(GROUND_ROW) - 46;
        const obstacleBody = Crafty.e("2D, DOM, Color")
            .attr({ x: baseX, y: baseY, w: 34, h: 46, z: 30 })
            .color("#65466f")
            .css({ "border-radius": "8px" });

        const obstacleCap = Crafty.e("2D, DOM, Color")
            .attr({ x: baseX + 3, y: baseY + 3, w: 28, h: 8, z: 31 })
            .color("#f38ba8")
            .css({ "border-radius": "4px", opacity: "0.85" });

        const obstacleSymbol = Crafty.e("2D, DOM, Text")
        ...
        const markerBg = Crafty.e("2D, DOM, Color")
            .attr({ x: colToX(col) + 6, y: baseY - 18, w: TILE - 12, h: 16, z: 33 })
            .color("#0e233a")
            .css({ "border-radius": "8px", border: "1px solid rgba(137,220,235,0.38)", opacity: "0.92" });

        const markerText = Crafty.e("2D, DOM, Text")
            .attr({ x: colToX(col) + 6, y: baseY - 15, w: TILE - 12, h: 12, z: 34 })
            .text("EVENTO")
            .textColor("#89dceb")
    ...
    function refreshRandomEventVisual(event) {
        if (event.resolved) {
            event.markerBg.color("#16352b").css({ border: "1px solid rgba(166,227,161,0.55)", opacity: "0.95" });
            event.markerText.text("RESOLVIDO").textColor("#a6e3a1");
            if (event.type === "crystal") {
                setEntityVisible(event.crystal, true);
                setEntityVisible(event.obstacleBody, false);
                setEntityVisible(event.obstacleCap, false);
                setEntityVisible(event.obstacleSymbol, false);
                event.crystal.css({ opacity: "0.35", filter: "saturate(0.7)" });
            } else {
                setEntityVisible(event.crystal, false);
                setEntityVisible(event.obstacleBody, true);
                setEntityVisible(event.obstacleCap, true);
                setEntityVisible(event.obstacleSymbol, true);
                event.obstacleBody.color("#4f5d73");
                event.obstacleCap.color("#a6e3a1");
                event.obstacleSymbol.text("✓").textColor("#a6e3a1");
            }
            return;
        }

        event.markerBg.color("#0e233a").css({ border: "1px solid rgba(137,220,235,0.38)", opacity: "0.92" });
        if (event.type === "crystal") {
            event.markerText.text("CRISTAL: interact()").textColor("#89dceb");
            setEntityVisible(event.crystal, true);
            setEntityVisible(event.obstacleBody, false);
            setEntityVisible(event.obstacleCap, false);
            setEntityVisible(event.obstacleSymbol, false);
            event.crystal.css({ opacity: "1", filter: "none" });
        } else {
            event.markerText.text("OBSTÁCULO: jump()").textColor("#f38ba8");
            setEntityVisible(event.crystal, false);
            setEntityVisible(event.obstacleBody, true);
            setEntityVisible(event.obstacleCap, true);
            setEntityVisible(event.obstacleSymbol, true);
            event.obstacleBody.color("#65466f").css({ filter: "none" });
            event.obstacleCap.color("#f38ba8").css({ filter: "none" });
            event.obstacleSymbol.text("!").textColor("#f9e2af").css({ filter: "none" });
        }
    }

    function buildRandomEvents() {
        for (let i = 0; i < RANDOM_EVENT_COLS.length; i++) {
            randomEvents.push(buildRandomEvent(RANDOM_EVENT_COLS[i]));
        }
    }

    function getNextPendingEvent() {
        for (let i = 0; i < randomEvents.length; i++) {
            if (!randomEvents[i].resolved) return randomEvents[i];
        }
        return null;
    }

    function rotateRandomEventsPattern() {
        const pattern = EVENT_PATTERNS[nextPatternIndex];
        nextPatternIndex = (nextPatternIndex + 1) % EVENT_PATTERNS.length;

        for (let i = 0; i < randomEvents.length; i++) {
            randomEvents[i].type = pattern[i % pattern.length];
            randomEvents[i].resolved = false;
            refreshRandomEventVisual(randomEvents[i]);
        }
    }

    function resetRandomEventsSamePattern() {
        for (let i = 0; i < randomEvents.length; i++) {
            randomEvents[i].resolved = false;
            refreshRandomEventVisual(randomEvents[i]);
        }
    ...
    function resolveRandomEvent(event, mode) {
        if (!event || event.resolved) return;
        event.resolved = true;
        refreshRandomEventVisual(event);
        if (mode === "interact") {
    ...
    function updateStepButtonState() {
        ...
        const total = stepSession.commands.length;
    ...
    function resetWorldForExecution(rotateType) {
        ...
        resetPlayerPosition();
        if (rotateType) {
            rotateRandomEventsPattern();
        } else {
            resetRandomEventsSamePattern();
        }
        hideAutocomplete();
    }
    ...
    function checkVictory() {
        ...
        if (getNextPendingEvent()) return false;
    ...
    function moveToCol(targetCol, id, moveSpeed) {
        const targetX = snapPixel(colToX(targetCol) + (TILE - PLAYER_W) / 2);
        ...
        return new Promise(function (resolve) {
            ...
            const handler = function () {
                ...
                player.x = snapPixel(player.x + direction * moveSpeed);
            };
    ...
    function hopInPlace(id) {
        ...
        return new Promise(function (resolve) {
            const handler = function () {
                ...
                player.y = snapPixel(startY - Math.sin(Math.PI * t) * 26);
            };
    ...
    function jumpToCol(targetCol, id, moveSpeed) {
        ...
        const jumpSpeed = Math.max(4, Math.round(moveSpeed * 1.75));
        ...
        return new Promise(function (resolve) {
            const handler = function () {
                ...
                player.x = snapPixel(player.x + direction * jumpSpeed);
                const progress = Math.abs(player.x - startX) / distance;
                const arc = 4 * progress * (1 - progress);
                player.y = snapPixel(startY - arc * 72);
            };
    ...
    function isCrystalAhead() {
        const pending = getNextPendingEvent();
        return !!pending &&
            pending.type === "crystal" &&
            currentCol + 1 === pending.col;
    }

    function isObstacleAhead() {
        const pending = getNextPendingEvent();
        return !!pending &&
            pending.type === "obstacle" &&
            currentCol + 1 === pending.col;
    }
    ...
    async function moveSteps(direction, steps, id, moveSpeed, lineNum) {
        for (let s = 0; s < steps; s++) {
            ...
            const pending = direction > 0 ? getNextPendingEvent() : null;
            if (pending) {
                if (pending.type === "obstacle" && nextCol === pending.col) {
                    throw new Error("Ln " + lineNum + ": obstáculo à frente. Use jump() ou if/else para desviar.");
                }
                if (pending.type === "obstacle" && nextCol > pending.col + 1) {
                    throw new Error("Ln " + lineNum + ": resolva o obstáculo aleatório antes de avançar.");
                }
                if (pending.type === "crystal" && nextCol > pending.col) {
    ...
    async function runInteract(lineNum, id, moveSpeed) {
        const pending = getNextPendingEvent();
        if (pending && pending.type === "crystal" && Math.abs(currentCol - pending.col) <= 1) {
            resolveRandomEvent(pending, "interact");
            checkVictory();
            return;
        }
    ...
    async function runJump(lineNum, id, moveSpeed) {
        const pending = getNextPendingEvent();
        if (pending && pending.type === "obstacle" && currentCol + 1 === pending.col) {
            ...
            markStepPassed(currentCol);
            resolveRandomEvent(pending, "jump");
            checkVictory();
            return;
        }
    ...
    function parseCommandsWithFeedback() {
        try {
            const commands = parseCode(editorEl.value);
            updateErrorStatus(0);
            return commands;
        } catch (err) {
    ...
    function createStepSessionFromEditor() {
        const commands = parseCommandsWithFeedback();
    ...
    async function executeNextStep() {
        ...
        if (stepSession.index >= stepSession.commands.length) {
            if (!hasWon) {
                if (getNextPendingEvent()) {
                    termLog("… Step concluído, mas ainda há eventos pendentes.", "term-output");
                } else {
                    termLog("… Step concluído. Eventos resolvidos; avance até a coluna " + GOAL_COL + ".", "term-output");
                }
            }
            return;
        }

        const localSession = stepSession;
        const cmd = localSession.commands[localSession.index];
        ...
        try {
            await executeCommand(cmd, localSession.id, localSession.moveSpeed);
        } catch (errRun) {
        ...
        if (stepSession.index >= stepSession.commands.length) {
            if (getNextPendingEvent()) {
                termLog("… Fim dos comandos no Step. Resolva os eventos aleatórios com if/else.", "term-output");
            } else {
                termLog("… Fim dos comandos no Step. Eventos resolvidos; avance até a coluna " + GOAL_COL + ".", "term-output");
            }
        }
    }
    ...
    async function executeProgram() {
        ...
        if (!hasWon) {
            if (getNextPendingEvent()) {
                termLog("… Resolva os eventos aleatórios usando if/else.", "term-output");
            } else {
                termLog("… Eventos resolvidos. Avance até a coluna " + GOAL_COL + ".", "term-output");
            }
        }
    }
